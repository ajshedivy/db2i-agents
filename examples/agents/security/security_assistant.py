...
async def run_db2i_cli(
    ...
    profile_security_agent.model = get_model(model_id=model_id)
    if debug_mode:
        profile_security_agent.debug_mode = debug_mode
    # Configure the CLI
    config = CLIConfig(
    ...
    cli = InteractiveCLI(agent=profile_security_agent, config=config, stream=stream)
    await cli.start()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-id", default="watsonx:meta-llama/llama-3-3-70b-instruct", help="Use Ollama model")
    parser.add_argument(
